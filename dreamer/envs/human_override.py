...
Controls:
  SPACE or S  — stop: end episode with penalty (car went off track)
  R           — reset: end episode cleanly (clean lap / goal reached)
  Q or ESC    — quit training
...
class HumanOverride:
    ...
    def __init__(self):
        ...
        print('    SPACE / S  →  STOP  (car off track)')
        print('    R          →  RESET (clean lap)')
        print('    Q / ESC    →  QUIT  (end training)')
        print('─' * 50 + '\n')
    ...
    def _start_listener(self):
        ...
        def on_press(key):
            ...
            with self._lock:
                if ch in ('s',) or key == keyboard.Key.space:
